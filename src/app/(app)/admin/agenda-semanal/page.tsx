...
export default function AdminAgendaSemanalPage() {
  ...
  if (loading) {
    return (
      <div className="card p-6 text-sm text-text-muted">Carregando permissões...</div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h2 className="text-xl font-semibold text-brand-900">Agenda semanal</h2>
        </div>
        <button
          type="button"
          onClick={openCreate}
      ...
      <div className="card p-4">
        <div className="flex items-center justify-between gap-3">
          <h3 className="text-sm font-semibold text-brand-900">Eventos cadastrados</h3>
          <button
            type="button"
            onClick={loadEvents}
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <label className="space-y-1 text-sm">
            <span className="text-text">Buscar evento</span>
            <input
              value={searchTerm}
            ...
            <tbody className="divide-y divide-surface">
              ...
              {filteredEvents.map((event) => (
                <tr key={event.id}>
                  ...
                  <td className="px-4 py-3 text-text">{event.start_time.slice(0, 5)}</td>
                  <td className="px-4 py-3 text-text">{event.title}</td>
                  <td className="px-4 py-3 text-text">{event.location ?? "-"}</td>
                  <td className="px-4 py-3 text-text">{event.notes ?? "-"}</td>
