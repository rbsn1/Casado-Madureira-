...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'ccm_discipleship_cases_fase_check'
      and conrelid = 'public.ccm_discipleship_cases'::regclass
  ) then
    alter table public.ccm_discipleship_cases
      add constraint ccm_discipleship_cases_fase_check
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'ccm_discipleship_cases_turno_origem_check'
      and conrelid = 'public.ccm_discipleship_cases'::regclass
  ) then
    alter table public.ccm_discipleship_cases
      add constraint ccm_discipleship_cases_turno_origem_check
      check (turno_origem in ('MANHA', 'NOITE', 'EVENTO') or turno_origem is null);
  end if;
end
$$;

create index if not exists ccm_discipleship_cases_fase_idx
  on public.ccm_discipleship_cases (congregation_id, fase, updated_at desc);

create index if not exists ccm_discipleship_cases_turno_origem_idx
  on public.ccm_discipleship_cases (congregation_id, turno_origem, updated_at desc);

create index if not exists ccm_discipleship_cases_modulo_atual_idx
  on public.ccm_discipleship_cases (modulo_atual_id);
...
from (
  ...
  from public.ccm_discipleship_cases dc_inner
) picked
